...
include_once RACINE_PATH.'/service/AdminService.php';
extract($_POST);
if(empty($_SESSION['idAdm'])){
if(isset($email)&&isset($password)){
   echo "<script>alert('level1');</script>";
    $ss = new AdminService();
    $p=sha1($password);
    $log=$ss->login($email,$p);// soit true soit false
    if(!empty($log) && !empty($log->getEmail()) && !empty($log->getPassword())){
        echo $log->getEmail()." and ".$log->getPassword();
        //if etat de compte active ou nn 
           
        $_SESSION['idAdm']=$log->getIduser();
...
<head>
	...
	<!-- Theme JS files -->
	<script type="text/javascript" src="assets/cp/js/plugins/forms/validation/validate.min.js"></script>
	<script type="text/javascript" src="assets/cp/js/plugins/forms/styling/uniform.min.js"></script>

	<script type="text/javascript" src="assets/cp/js/core/app.js"></script>
	<script type="text/javascript" src="assets/cp/js/pages/login_validation.js"></script>
	<!-- /theme JS files -->
					...
					<!-- Form with validation -->
						...
						<div class="panel panel-body login-form">
							...
							<div class="form-group has-feedback has-feedback-left">
                                                            <input type="email" class="form-control" placeholder="identiant" name="email" required="required">
